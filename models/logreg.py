...
train_grad = "../data/train/grad"
train_cluster = "../data/train/cluster"
test_grad= "../data/test/grad"
test_cluster= "../data/test/cluster"
image_size = 128

for image in tqdm(os.listdir(train_grad)): 
...
for image2 in tqdm(os.listdir(train_cluster)): 
    path = os.path.join(train_cluster, image2)
...
def test_data():
    test_data_messy = [] 
    test_data_clean=[]
    for image1 in tqdm(os.listdir(test_grad)): 
        path = os.path.join(test_grad, image1)
        img1 = cv2.imread(path, cv2.IMREAD_GRAYSCALE) 
        img1 = cv2.resize(img1, (image_size, image_size))
        test_data_messy.append(img1) 
    for image2 in tqdm(os.listdir(test_cluster)): 
        path = os.path.join(test_cluster, image2)
        img2 = cv2.imread(path, cv2.IMREAD_GRAYSCALE) 
        img2 = cv2.resize(img2, (image_size, image_size))
